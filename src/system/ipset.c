...
static void create_ipset(const char *ipset_bin, const char *list_name, int is_v6) {
    int r;
    if (is_v6) {
        char *argv[] = { (char *)ipset_bin, (char *)"create", (char *)list_name, (char *)"hash:ip", (char *)"family", (char *)"inet6", (char *)"timeout", (char *)"3600", (char *)"-exist", NULL };
        r = run_ipset(ipset_bin, argv);
    } else {
        char *argv[] = { (char *)ipset_bin, (char *)"create", (char *)list_name, (char *)"hash:ip", (char *)"timeout", (char *)"3600", (char *)"-exist", NULL };
        r = run_ipset(ipset_bin, argv);
    }
...
void ipset_init(void) {
    ...
    if (access(ipset_bin, X_OK) != 0) {
        fprintf(stderr, "[AutoBan] WARNING: %s không tồn tại hoặc không thực thi được. Bỏ qua ipset_init.\n", ipset_bin);
        return;
    }
