...
static const char *determine_block_cmd(AppConfig *config, const char *ip, const char *server_name,
                                       int offense_count) {
    int is_v6 = (strchr(ip, ':') != NULL);
    int retries = config->max_ban_retries > 0 ? config->max_ban_retries : 3;

    if (offense_count >= retries &&
        (strlen(config->permanent_ban_cmd) > 0 || strlen(config->permanent_ban_cmd_v6) > 0)) {
        printf("[AutoBan] Multi-Level Triggered! IP %s đã vi phạm %d lần. Áp dụng lệnh nâng cao.\n",
               ip, offense_count);
        const char *cmd = (is_v6 && strlen(config->permanent_ban_cmd_v6) > 0)
                              ? config->permanent_ban_cmd_v6
                              : config->permanent_ban_cmd;
        if (strlen(cmd) == 0 && strlen(config->permanent_ban_cmd) > 0)
            cmd = config->permanent_ban_cmd;
        return cmd;
    }
...
static int build_job_command(char *dest, size_t dest_size, const char *cmd_template, const char *ip,
                             int ban_time) {
...
void block_ip(AppConfig *config, const char *ip, const char *server_name, int is_synced) {
    if (!block_is_valid_ip(ip)) {
        fprintf(
            stderr,
            "[AutoBan] SECURITY WARNING: IP Parse Error hoặc mã độc detected '%s'. Không ban.\n",
            ip);
        return;
    }

    if (is_whitelisted(config, ip))
        return;

    // [CWE-362] Atomic check-and-mark using a static mutex to prevent duplicate enqueue
    // between multiple journal reader threads
    static pthread_mutex_t block_dedup_mutex = PTHREAD_MUTEX_INITIALIZER;
    pthread_mutex_lock(&block_dedup_mutex);
    if (cache_is_recently_blocked(ip, server_name)) {
        cache_inc_cache_hits();
        pthread_mutex_unlock(&block_dedup_mutex);
        return;
    }
    // Pre-mark the cache to close the race window before releasing the lock
    cache_mark_blocked(ip, server_name);
    pthread_mutex_unlock(&block_dedup_mutex);
    ...
    BlockJob job;
    if (build_job_command(job.cmd, sizeof(job.cmd), block_cmd, ip, ban_time) < 0)
        return;
