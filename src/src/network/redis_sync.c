...
static void load_redis_config(AutobanContext *ctx, int *enabled, char *host, int *port,
                              char *password, char *channel) {
    pthread_mutex_lock(ctx->config_mutex);
    if (*ctx->config_ptr) {
        *enabled = (*ctx->config_ptr)->redis_enabled;
        *port = (*ctx->config_ptr)->redis_port;
        strncpy(host, (*ctx->config_ptr)->redis_host, MAX_STRING - 1);
        host[MAX_STRING - 1] = '\0';
        strncpy(password, (*ctx->config_ptr)->redis_password, MAX_STRING - 1);
        password[MAX_STRING - 1] = '\0';
        strncpy(channel, (*ctx->config_ptr)->redis_channel, MAX_STRING - 1);
        channel[MAX_STRING - 1] = '\0';
    } else {
        *enabled = 0;
        host[0] = password[0] = channel[0] = '\0';
        *port = 6379;
    }
...
static redisContext *connect_to_redis(const char *host, int port, const char *password) {
    ...
    if (c == NULL || c->err) {
        fprintf(stderr,
                "[AutoBan Redis] Lỗi kết nối Subscriber tới Redis %s:%d. Thử lại sau 5s...\n", host,
                port);
        if (c)
            redisFree(c);
        return NULL;
