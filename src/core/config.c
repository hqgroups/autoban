...
static int is_dangerous_cmd(const char *cmd) {
    if (cmd[0] != '/') {
        fprintf(stderr, "[AutoBan] SECURITY: Lệnh '%s' phải bắt đầu bằng đường dẫn tuyệt đối (ví dụ /usr/sbin/ipset)\n", cmd);
        return 1;
    }

    if (strstr(cmd, "rm ") || strstr(cmd, "mkfs") ||
        strstr(cmd, "mv ") || strstr(cmd, "cp ") ||
        strstr(cmd, "chmod ") || strstr(cmd, "chown") ||
        strstr(cmd, ">") || strstr(cmd, ">>") ||
        strstr(cmd, "|") || strstr(cmd, "&") || strstr(cmd, ";"))
        return 1;
    
    if (strstr(cmd, "wget") || strstr(cmd, "curl") ||
        strstr(cmd, "base64") || strstr(cmd, "nc ") || strstr(cmd, " telnet"))
        return 1;
    return 0;
}
static int validate_template(const char *template) {
    ...
    if (strlen(tmp) > 0) {
        if (access(tmp, X_OK) != 0) {
            fprintf(stderr, "[AutoBan] SECURITY ERROR: Lệnh thực thi '%s' không tồn tại hoặc không có quyền Execute.\n", tmp);
            return 0;
        }
    }
...
static void handle_global_block_cmd(AppConfig *config, const char *val) {
    if (is_dangerous_cmd(val)) {
        fprintf(stderr, "[AutoBan] SECURITY WARNING: Lệnh block_cmd cấu hình toàn cầu có chứa pattern nguy hiểm. Bỏ qua.\n");
    } else if (validate_template(val)) {
        strncpy(config->block_cmd, val, MAX_STRING - 1);
        config->block_cmd[MAX_STRING - 1] = '\0';
    } else {
        fprintf(stderr, "[AutoBan] WARNING: Lệnh block_cmd thiếu %%s, %%d hoặc định dạng sai. Bỏ qua.\n");
    }
}

static void handle_global_block_cmd_v6(AppConfig *config, const char *val) {
    if (is_dangerous_cmd(val)) {
        fprintf(stderr, "[AutoBan] SECURITY WARNING: Lệnh block_cmd_v6 cấu hình toàn cầu có chứa pattern nguy hiểm. Bỏ qua.\n");
    } else if (validate_template(val)) {
        strncpy(config->block_cmd_v6, val, MAX_STRING - 1);
        config->block_cmd_v6[MAX_STRING - 1] = '\0';
    } else {
        fprintf(stderr, "[AutoBan] WARNING: Lệnh block_cmd_v6 thiếu %%s, %%d hoặc định dạng sai. Bỏ qua.\n");
    }
}
...
static const ConfigHandler global_handlers[] = {
    ...
    { "permanent_ban_cmd_v6", TYPE_STRING, offsetof(AppConfig, permanent_ban_cmd_v6), MAX_STRING, NULL },
...
static void process_handlers(AppConfig *config, const ConfigHandler *handlers, const char *key, const char *val) {
...
static void handle_site_block_cmd(SiteConfig *config, const char *server_name, const char *val) {
    if (is_dangerous_cmd(val)) {
        fprintf(stderr, "[AutoBan] SECURITY WARNING: Lệnh block_cmd của site %s chứa từ ngữ cấm. Bỏ qua.\n", server_name);
    } else if (validate_template(val)) {
        strncpy(config->block_cmd, val, MAX_STRING - 1);
        config->block_cmd[MAX_STRING - 1] = '\0';
    } else {
        fprintf(stderr, "[AutoBan] WARNING: Lệnh block_cmd site %s thiếu %%s, %%d hoặc định dạng sai. Bỏ qua.\n", server_name);
    }
}

static void handle_site_block_cmd_v6(SiteConfig *config, const char *server_name, const char *val) {
    if (is_dangerous_cmd(val)) {
        fprintf(stderr, "[AutoBan] SECURITY WARNING: Lệnh block_cmd_v6 của site %s chứa từ ngữ cấm. Bỏ qua.\n", server_name);
    } else if (validate_template(val)) {
        strncpy(config->block_cmd_v6, val, MAX_STRING - 1);
        config->block_cmd_v6[MAX_STRING - 1] = '\0';
    } else {
        fprintf(stderr, "[AutoBan] WARNING: Lệnh block_cmd_v6 site %s thiếu %%s, %%d hoặc định dạng sai. Bỏ qua.\n", server_name);
    }
}
...
static void process_site_handlers(SiteConfig *config, const char *server_name, const SiteConfigHandler *handlers, const char *key, const char *val) {
    for (int i = 0; handlers[i].key != NULL; i++) {
        if (strcmp(key, handlers[i].key) == 0) {
            if (handlers[i].type == TYPE_INT) {
                int *ptr = (int *)((char *)config + handlers[i].offset);
...
static void set_default_config(AppConfig *config) {
    strcpy(config->block_cmd, "ipset add autoban_list %s timeout %d");
    strcpy(config->block_cmd_v6, "ipset add autoban_list_v6 %s timeout %d");
...
static void parse_whitelist_line(AppConfig *config, const char *val) {
    if (config->whitelist_count >= config->whitelist_capacity) {
        ...
        CidrEntry *new_whitelist = realloc(config->whitelist, config->whitelist_capacity * sizeof(CidrEntry));
        if (!new_whitelist) {
            config->whitelist_capacity /= 2;
            return;
        }
    ...
    if (inet_pton(AF_INET, ip_buf, &addr4) == 1) {
        ...
        config->whitelist[config->whitelist_count].prefix = (prefix >= 0 && prefix <= 32) ? prefix : 32;
        ...
        config->whitelist[config->whitelist_count].prefix = (prefix >= 0 && prefix <= 128) ? prefix : 128;
        config->whitelist_count++;
    } else {
...
static void parse_site_config(AppConfig *config, const char *server_name, const char *key, const char *val) {
    int site_idx = -1;
    ...
    if (site_idx == -1) {
        if (config->site_count >= config->site_capacity) {
            ...
            SiteConfig *new_sites = realloc(config->sites, config->site_capacity * sizeof(SiteConfig));
            if (!new_sites) {
                config->site_capacity /= 2;
                return;
            }
...
int load_config(const char *filename, AppConfig *config) {
    ...
    if (!config->whitelist) {
        free(config->sites); config->sites = NULL;
        fclose(f);
        return -1;
    }
        ...
        if (eq) {
            *eq = '\0';
            ...
            if (strcmp(current_section, "global") == 0) {
                ...
                if (strcmp(key, "allow") == 0) parse_whitelist_line(config, val);
...
int is_whitelisted(AppConfig *config, const char *ip) {
    if (!config || config->whitelist_count <= 0) return 0;
    ...
    for (int i = 0; i < config->whitelist_count; i++) {
        ...
        if (is_v4 && entry->family == AF_INET) {
            ...
            for (int j = 0; j < 16; j++) {
                if (prefix >= 8) {
                    if (addr6.s6_addr[j] != entry->addr.v6.s6_addr[j]) { match = 0; break; }
                    prefix -= 8;
                } else if (prefix > 0) {
                    unsigned char mask = (unsigned char)(0xFF00 >> prefix);
                    if ((addr6.s6_addr[j] & mask) != (entry->addr.v6.s6_addr[j] & mask)) { match = 0; break; }
                    prefix = 0;
                } else {
                    break;
                }
...
void free_config(AppConfig *config) {
    if (config->sites) free(config->sites);
