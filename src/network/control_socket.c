    ...
    if (bind(server_fd, (struct sockaddr *)&addr, sizeof(addr)) == 0) {
        if (listen(server_fd, 3) == 0) {
            if (chmod(socket_path, 0600) != 0) {
                perror("[AutoBan] WARNING: Không thể thiết lập quyền 0600 cho Control Socket");
            }
    ...
    if (n > 0) {
        ...
        if (strcmp(buf, "status") == 0) {
            ...
            snprintf(resp, sizeof(resp),
                ...
                "  Whitelist IPs: %d\n"
                "  Sites        : %d\n",
                AUTOBAN_VERSION, cache_get_metrics_total_blocks(), cache_get_metrics_cache_hits(),
                q_len, MAX_QUEUE_SIZE, w_count, s_count);
